...
const Checkout = () => {
  ...
  const handlePaymentMethodSelect = (method) => {
    ...
    else if (method === 'demo') setShowDemoPayment(true);
  };

  // ── Demo Payment handlers ───────────────────────────────────────
  const handleDemoSuccess = (data) => {
  ...
  const handleDemoFailure = (message, data) => {
    setShowDemoPayment(false);
    if (data?.orderId) {
      navigate('/payment-success', {
        state: {
      ...
      setError(message || 'Demo payment failed. Please try again.');
    }
  };
      ...
      <div className="checkout-container">
        ...
        <div className="demo-mode-banner">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" style={{ flexShrink: 0 }}>
            <circle cx="12" cy="12" r="10" stroke="#00B0FF" strokeWidth="1.8" />
            <path d="M12 8v4M12 16h.01" stroke="#00B0FF" strokeWidth="2" strokeLinecap="round" />
          </svg>
          <span>
            <strong>Demo Payment Available</strong> — Select &ldquo;Demo Card / UPI&rdquo; after filling the form to test the payment flow without any real transaction.
          </span>
        </div>
                ...
                {showPaymentDropdown && (
                  <div className="payment-dropdown">
                    ...
                    <div className="payment-options">
                      ...
                      {/* Online / Razorpay */}
                      <button onClick={() => handlePaymentMethodSelect('online')}
                        className="payment-option" disabled={isBusy} type="button">
                        <span className="payment-icon">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                      ...
                      {/* Demo Payment */}
                      <button onClick={() => handlePaymentMethodSelect('demo')}
                        className="payment-option payment-option-demo" disabled={isBusy} type="button">
                        <span className="payment-icon">
                          <svg width="32" height="32" viewBox="0 0 24 24" fill="none">
                            <rect x="3" y="5" width="18" height="14" rx="2" stroke="#00B0FF" strokeWidth="2" />
                            <path d="M3 10H21" stroke="#00B0FF" strokeWidth="2" />
                            <path d="M7 15H10M14 15H17" stroke="#00B0FF" strokeWidth="2" strokeLinecap="round" />
                            <circle cx="19" cy="6" r="4" fill="#00B0FF" />
                            <path d="M17.5 6h3M19 4.5v3" stroke="white" strokeWidth="1.5" strokeLinecap="round" />
                          </svg>
                        </span>
                        <div className="payment-text">
                          <strong>Demo Card / UPI <span className="demo-badge-inline">TEST MODE</span></strong>
                          <small>Simulate payment — no real charge</small>
                        </div>
                      </button>
                    </div>
                  </div>
        ...
        {showDemoPayment && (
          <DemoPayment
            orderPayload={buildOrderPayload('DEMO')}
